...
: ld+,   12 4nybbles, ; \ ( z y x -- ) z<-[y] y+=x (load from memory and inc/dec pointer)
: st+,   13 4nybbles, ; \ ( z y x -- ) z->[y] y+=x (store to memory and inc/dec pointer)
: ldc,   14 2nybbles, c, ; \ ( v x -- ) x=v (load constant signed v into x)
