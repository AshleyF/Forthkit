        ...
        if ((c & 1) == 0) { // call?
            ...
            for (short slot = 0; slot < 15; slot += 5) {
                ...
                switch (i) {
                    ...
                    case 17: mem[Y] = X & 0xFF; mem[Y + 1] = X >> 8; STACK(d, -1); X += 2; break; // ST16+ - Store cell at address, and increment over
                    ...
                    case 21: if (X == 0) { p += (signed char)mem[p]; slot = 15; } else { p++; } break; // 0JUMP - Jump to relative to offset in next byte if T = 0
